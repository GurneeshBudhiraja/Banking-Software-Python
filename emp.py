  ...
  def add_cx():
    try:
      ...
      with open('cx.csv','a',newline='') as file:
        writer=csv.DictWriter(file,fieldnames=['name','postal code','balance','status'])
        writer.writerow({'name':cx_name,'postal code':cx_postal, 'balance':'0','status':'Active'})
...
class Head(Manager):
  @staticmethod
  def display():
    print(tabulate([['Employee'],['Customer']],headers=['Option Name'],tablefmt='pretty',showindex='always'))
    ...
    if (head_choice==0):
      ...
      if emp:
        ...
        for row in emp:
          print(f'{tabulate([row.values()], tablefmt='pretty')}')
